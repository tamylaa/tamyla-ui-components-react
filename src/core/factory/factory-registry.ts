...
export class FactoryRegistry {
  ...
  private initializeFactories(): void {
    ...
    this.registerFactory('InputGroup', () => this.safeCall(factoryImporter.getFactory('InputGroupFactory')));
    ...
    this.registerFactory('ContentCard', () => this.safeCall(factoryImporter.getFactory('ContentCardFactory')));
    this.registerFactory('FileList', () => this.safeCall(factoryImporter.getFactory('FileListFactory')));
    this.registerFactory('Notification', () => this.safeCall(factoryImporter.getFactory('NotificationFactory')));
    this.registerFactory('SearchInterface', () => this.safeCall(factoryImporter.getFactory('SearchInterfaceFactory')));
    ...
    this.registerFactory('EnhancedSearch', () => this.safeCall(factoryImporter.getFactory('EnhancedSearchApplicationFactory')));
    this.registerFactory('CampaignSelector', () => this.safeCall(factoryImporter.getFactory('CampaignSelectorSystem')));
    this.registerFactory('ContentManager', () => this.safeCall(factoryImporter.getFactory('ContentManagerApplicationFactory')));
    this.registerFactory('TamylaUI', () => this.safeCall(factoryImporter.getFactory('TamylaUISystem')));

    // Organism factories
    this.registerFactory('OrganismFactory', () => this.safeCall(factoryImporter.getFactory('OrganismFactory')));
    this.registerFactory('OrganismTemplates', () => this.safeCall(factoryImporter.getFactory('OrganismTemplates')));

    // Reward system
    this.registerFactory('Reward', () => this.safeCall(factoryImporter.getFactory('RewardSystem')));
  }
  ...
  private safeCall(factory: any, method?: string): any {
    try {
      ...
      // If no method specified, return the factory directly (for function factories)
      if (typeof factory === 'function') {
        return factory;
      }

      console.warn('FactoryRegistry: Factory is not a function and no method specified');
      return null;
    } catch (error) {
